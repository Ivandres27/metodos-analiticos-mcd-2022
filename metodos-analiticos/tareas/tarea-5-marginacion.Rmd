...
**La dimensión de cada uno está en función de la matriz "marg_tabla_esc" con las columnas de U vectores propios izquierdos y las columnas de V vectores propios derechos**
...
R <- X - X_1
Frobenius_R<-norm(R,"F")
dim(marg_tabla)
```
Grafica los valores de la aproximación $X_1$ vs $X$

```{r}
qplot(as.numeric(X_1), as.numeric(as.matrix(X))) + geom_abline(colour='red')
```
...
**Los pesos de las varaibles para ponderar cada uno al calcular el Indice**
...
**En la aproximación con la matriz de rango 1 los valores de v_1 son los pesos que aproximan cada variable en la dirección de u_1, cada valor de v representa el peso de u_1 como aproximación de cada variable de marginación**

**Pregunta 5**: ¿Puedes reconstruir el índice oficial a partir de la componente que obtuviste?

**Si aproximando las variables con la matriz X_1 y calculando el Indice a partir de ellas **
